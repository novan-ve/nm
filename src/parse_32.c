...
int parse_32(char *filename, char *data, size_t len) {
    ...
    if (header->e_shoff + sizeof(Elf32_Shdr) * header->e_shnum > len) {
        return (2);
    }

    Elf32_Shdr *sections = (Elf32_Shdr*)(data + header->e_shoff);
    ...
    for (int i = 0; i < header->e_shnum; i++) {
        ...
        if (sections[i].sh_type == SHT_SYMTAB) {
            ...
            if (symtab_hdr->sh_entsize != sizeof(Elf32_Sym)) {
                return (2);
            }
        }
    }
    ...
    Elf32_Sym *entries = (Elf32_Sym*)(data + symtab_hdr->sh_offset);
    uint32_t strtab_len = sections[strtab_ndx].sh_size;
    uint32_t valid_sym = 0;

    uint32_t total_sym = symtab_hdr->sh_size / sizeof(Elf32_Sym);
    t_symbol symbols[total_sym];

    // Starting at 1 because the first symbol should be empty in a valid ELF file
    for (uint32_t i = 1; i < total_sym; i++) {
        symbols[valid_sym].name = NULL;
        symbols[valid_sym].value = entries[i].st_value;
        symbols[valid_sym].size = entries[i].st_size;
        symbols[valid_sym].type = get_symbol_type_32(entries[i], sections, header->e_shnum);
        ...
        if (ELF32_ST_TYPE(entries[i].st_info) != STT_SECTION &&
            ELF32_ST_TYPE(entries[i].st_info) != STT_FILE) {
